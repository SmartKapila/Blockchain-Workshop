...
document.addEventListener(
	...
	async (e) => {
		...
		const question = await Blockchain_backend.getQuestion();
		document.getElementById("question").innerText = question;

		const voteCounts = await Blockchain_backend.getVotes();
...
pollForm.addEventListener(
	...
	async (e) => {
		...
		const updatedVoteCounts = await Blockchain_backend.vote(checkedValue);
...
resetButton.addEventListener(
	...
	async (e) => {
		...
		await Blockchain_backend.resetVotes();
		const voteCounts = await Blockchain_backend.getVotes();
		updateLocalVoteCounts(voteCounts);
